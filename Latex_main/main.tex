...
nondegenerate, but the median coefficient of variation is 0.448; reporting
each parameter as its bootstrap median with a percentile interval (so the point
estimate is bracketed by construction) shows that the series-resistance
interval is fixed by the prior box, not the data (100\% of draws pinned to an
edge).  Eigendecomposition of the Fisher matrices makes the sloppiness
quantitative: the spectra span on average $\sim$37 orders of magnitude with a
single dominant stiff direction, so only a few effective combinations are
determined.  A formal face-centered response-surface test with center-point
lack-of-fit finds that only $V_{0,\mathrm{SET}}$ carries a statistically
significant deposition dependence (oxygen and power, $R^2=0.75$), and a
variance decomposition over the replicated center points shows cycle-to-cycle
variation dominates device-to-device variation for every parameter.  Mechanism
classification shows Schottky-dominant post-RESET high-resistance-state
conduction in 92.5--100\% of cycles for all twelve conditions, exposing a
structural mismatch with the Stanford bulk current law.  The resulting
deliverable is therefore a behavioral compact-model calibration accompanied by
explicit fit, mechanism, identifiability, and variability evidence.
\end{abstract}
...
to an individual coordinate.  To quantify the sloppiness rather than merely
assert it, we eigendecompose the active-parameter Fisher matrix per condition:
the eigenvalue spectrum measures the dynamic range of constraint, and the
eigenvector of the largest eigenvalue is the stiff (best-determined) parameter
combination \cite{transtrum2015sloppiness}.

Because the deposition design (Table~\ref{tab:doe}) is a face-centered
factorial with four replicated center points, deposition dependence of each
identifiable parameter is tested formally: a coded second-order response
surface is fit by least squares, coefficient $p$-values are obtained, and the
center replicates serve as pure error in an $F$ lack-of-fit test.
...
Each parameter is reported as the bootstrap median with a 2.5/97.5 percentile
interval, so the point estimate is bracketed by its own interval; the
full-budget refined optimum, which the reduced-budget bootstrap optima track
only approximately (outside the interval for 7 of 192 parameter--conditions),
is kept only as a reference.  For each parameter we also report the fraction of
bootstrap draws pinned within 1\% of a prior-box edge, flagging intervals whose
width is set by the box rather than the data.  These intervals reflect
cycle-to-cycle variability under the fitting protocol and make no claim about
device-to-device or wafer-level populations; the four replicated center-point
devices (S9--S12) are used separately to split each parameter's variance into
cycle-to-cycle and device-to-device components.
...
all conditions in 92.5--100\% of cycles.  Classifying the Schottky window of
every measured cycle and bootstrapping over cycles, the per-condition median
$\varepsilon_r^{\mathrm{dyn}}$ spans 5.3--24.7 under
$t_{\mathrm{ox}}=\SI{7}{\nano\meter}$ (454 Schottky cycles in total), every
value inside the deliberately broad admissibility interval; this makes the
interface permittivity a cycle-resolved, model-free process observable rather
than a single-curve estimate.
...
branch scales---voltage curvature and switching-field combinations---more
reliably than the decomposition into $I_0$, $g_0$, activation energies,
velocity prefactors, and gap geometry.  Series resistance is the clearest case
where the \emph{local} Fisher diagnostic and the \emph{global} bootstrap
diverge: it is the most recurrent locally identifiable coordinate, yet under
cycle resampling every bootstrap draw rails against a prior-box edge (pinned
fraction $1.00$), so its apparent constraint is an artifact of the box, not the
data.  Eigendecomposing the Fisher matrices makes the structure explicit
(Fig.~\ref{fig:sloppy}): the spectra span on average $\sim$37 orders of
magnitude (11.5--69.3 across conditions), with only 5--10 of the 16 active
directions within six decades of the stiffest, and a single stiff direction
dominated by the coupled current-scale/gap coordinates.  This is the eigenbasis
statement of why only a few effective combinations---not the individual
microscopic parameters---are recoverable, which the cycle bootstrap of
Section~\ref{sec:res-variability} independently confirms.
...
eigenvalue spectra for all twelve conditions, falling over tens of decades.
Right: mean participation of each active parameter in the stiffest
(best-determined) eigen-direction---a coupled current-scale/gap combination,
not any individual microscopic parameter.}
\label{fig:sloppy}
\end{figure}
...
The deposition design (Table~\ref{tab:doe}) lets us ask which of these
repeatable coordinates actually moves with process.  Fitting a coded
second-order response surface with a center-point lack-of-fit test
(Table~\ref{tab:doe-rsm}), only $V_{0,\mathrm{SET}}$ shows a statistically
significant deposition dependence---rising with oxygen ($p=0.027$) and falling
with power ($p=0.044$), $R^2=0.75$, no significant lack-of-fit.
$V_{0,\mathrm{RESET}}$ does not reach significance, and $\log_{10}R_s$ carries
no significant term ($R^2=0.24$), consistent with its box-limited interval.
Thus, of the full Stanford parameter table, exactly one coordinate is currently
admissible as a process response.

\begin{table}[!t]
\centering
\caption{Coded response-surface fit of the identifiable parameters versus
oxygen and power.  $p$-values are for the linear coefficients; lack-of-fit
(LoF) uses the four center-point replicates as pure error.}
\label{tab:doe-rsm}
\footnotesize
...
Finally, the four replicated center-point devices (S9--S12) separate
cycle-to-cycle from device-to-device variability (Fig.~\ref{fig:varcomp}).  For
every parameter the cycle-to-cycle coefficient of variation exceeds the
device-to-device value and the intraclass device fraction is below 0.2 (e.g.\
0.04 for $V_{0,\mathrm{SET}}$, 0.06 for $R_s$): the cycle bootstrap captures the
dominant uncertainty, and the spread among nominally identical devices is
mostly cycling rather than fabrication.
...
(S9--S12).  Cycle-to-cycle spread dominates for every parameter.}
\label{fig:varcomp}
\end{figure}
...
Table~\ref{tab:doe}), test the identifiable coordinates formally rather than
reading trends off a plot: here only $V_{0,\mathrm{SET}}$ survives a
center-point lack-of-fit test as a process-dependent response
(Table~\ref{tab:doe-rsm}), while raw columns such as $R_s$ mix data- and
prior-driven information and must not be regressed against deposition.
...
effective time scale.  Fisher analysis is local; it is now reported together
with its eigenspectrum and with profile-likelihood scans completed for S1--S4
(88\% flat/non-identifiable), but a future release should complete profile
likelihood at one common optimum for all twelve conditions.  Bootstrap
intervals are cycle-level for one selected device per non-center condition; the
variance decomposition over the replicated center points (S9--S12) shows
cycle-to-cycle variation dominates device-to-device variation, but full
device-to-device and wafer-level statistics would require a multi-device
extension.  Finally, the reset-side $\gamma_0$ is structurally overridden under
negative bias in the released Verilog-A, so it is retained for model
