...
variation is 0.448, and the Fisher information spectra are extremely
\emph{sloppy}, spanning on average $\sim$37 orders of magnitude with a single
dominant stiff (identifiable) direction.  Reporting each parameter as its
bootstrap median with a percentile interval---rather than a local Fisher
error---removes point/interval inconsistencies and exposes that the
series-resistance interval is fixed by the prior box (100\% of draws pinned to
an edge) rather than by the data.  Consequently, of the repeatable and
physically interpretable coordinates, only $V_{0,\mathrm{SET}}$ shows a
statistically significant deposition dependence (oxygen and power,
$R^2=0.75$, no significant lack-of-fit), whereas series resistance shows none.
Replicated center-point devices further show that cycle-to-cycle variation
dominates device-to-device variation for every parameter (intraclass device
fraction $<0.2$).  Finally, the post-RESET high-resistance state is
Schottky-dominant in 92.5--100\% of cycles for all twelve conditions, with
cycle-bootstrap dynamic permittivities (medians 5.3--24.7) inside the
physically admissible range---a model-free interface fingerprint that survives
where the compact-model parameters do not.  Generalization, identifiability,
and mechanism validity should therefore be reported together as the
...
\begin{enumerate}
  ...
  \item \emph{Bootstrap-led operational identifiability with a sloppiness
        spectrum.}  Cycle-level cluster-bootstrap intervals are the primary
        identifiability evidence; each parameter is reported as a bootstrap
        median with a percentile interval (so the point estimate is bracketed
        by construction), and the fraction of draws pinned to the prior box is
        reported so that box-limited intervals are not mistaken for data
        constraints.  A Fisher eigenvalue spectrum quantifies the model
        \emph{sloppiness} directly and identifies the stiff parameter
        combination, with profile-likelihood scans as corroboration.
  \item \emph{Statistically tested deposition-resolved extraction.}  Across the
        twelve conditions we fit a face-centered response surface with a
        center-point lack-of-fit test and find that only $V_{0,\mathrm{SET}}$
        carries a statistically significant deposition dependence (oxygen and
        power, $p<0.05$, $R^2=0.75$), whereas series resistance---whose
        bootstrap interval is set entirely by the prior box---does not.  A
        variance decomposition over the replicated center points shows
        cycle-to-cycle variation dominates device-to-device variation for every
        parameter.  The post-RESET HRS is additionally Schottky-limited---%
        outside the Stanford bulk law---in nearly all cycles of all conditions,
        and we report its dynamic permittivity as a cycle-resolved, model-free
        process fingerprint.
\end{enumerate}
...
mechanism-stability table and, for each cycle whose post-RESET HRS window is
Schottky, a per-cycle dynamic permittivity $\varepsilon_r^{\mathrm{dyn}}$; the
distribution over cycles gives a model-free interface observable with a
cycle-bootstrap confidence interval (Section~\ref{sec:res-trends}).
...
per-parameter claims are made only where the bootstrap agrees.  To make the
sloppiness quantitative we eigendecompose the active-parameter Fisher matrix
per condition: the eigenvalue spectrum measures the dynamic range of
constraint, and the eigenvector of the largest eigenvalue gives the
\emph{stiff} (best-determined) parameter combination, following the
sloppy-model framework \cite{transtrum2015sloppiness,gutenkunst2007}.

Finally, because the deposition design (Table~\ref{tab:doe}) is a face-centered
factorial with four replicated center points, we test deposition dependence of
each \emph{identifiable} parameter formally rather than by eye: we fit a
second-order coded response surface
$y=\beta_0+\beta_1x_{\mathrm{O}_2}+\beta_2x_{P}+\beta_3x_{\mathrm{O}_2}x_{P}
+\beta_4x_{\mathrm{O}_2}^2+\beta_5x_{P}^2$
by least squares, obtain coefficient $p$-values, and use the center-point
replicates as pure error in an $F$ lack-of-fit test \cite{efron1994bootstrap}.
...
evaluations) \cite{efron1994bootstrap}.  Each parameter is then reported as the
\emph{bootstrap median} with a 2.5/97.5 percentile interval, so the reported
point estimate is bracketed by its own interval by construction.  We do not use
the full-budget refined optimum as the plotted estimate, because the
reduced-budget bootstrap optima are mildly biased relative to it (its value
falls outside the percentile interval for 7 of 192 parameter--conditions); the
refined optimum is retained only as a reference column.  For every active
parameter we additionally report the fraction of bootstrap draws pinned within
1\% of a prior-box edge, so an interval whose width is set by the box rather
than by the data is flagged explicitly.  These intervals reflect cycle-to-cycle
variability under the fitting protocol and make no claim about device-to-device
or wafer-level populations; however, because four center-point devices
(S9--S12) are nominally identical, we use them to decompose the variance of
each parameter into a cycle-to-cycle component (the mean within-device bootstrap
variance) and a device-to-device component (the variance of the four device
medians), summarized by the intraclass device fraction
$\mathrm{ICC}=\sigma^2_{\mathrm{dev}}/(\sigma^2_{\mathrm{dev}}+\sigma^2_{\mathrm{cyc}})$.
...
in-sample means of 0.530 and 0.761.  The \emph{median} per-condition
generalization gap is only $+0.011$ decade on both branches; the slightly
larger mean RESET gap ($+0.040$ decade) is dominated by a single outlier, S1,
whose RESET held-out median (0.999) exceeds its in-sample value (0.689) by
0.310 decade ($+45\%$).  We flag S1 RESET explicitly rather than averaging it
away: it is the one condition where a single medoid cycle is not fully
representative of the RESET population, and it coincides with the noisiest
low-power, oxygen-poor corner.  For the other eleven conditions the gap stays
within a few hundredths of a decade, so apart from S1 RESET the calibration is
a property of the device's cycle population, not an artifact of medoid
selection: the medoid is simply the most central member of a distribution the
model reproduces uniformly.
...
Two safeguards keep these intervals honest.  First, the reported point estimate
is the bootstrap median and the interval is its 2.5/97.5 percentile, so each
point lies inside its own interval; using the full-budget refined optimum
instead would place it outside the bootstrap interval for 7 of 192
parameter--conditions, an inconsistency we therefore avoid.  Second, we report
the fraction of bootstrap draws pinned to a prior-box edge.  This is decisive
for $R_s$: \emph{every} bootstrap draw rails against a box edge (pinned
fraction $1.00$), so its one-to-two-decade interval is set by the box, not by
the data---a quantitative statement of its non-identifiability that the velocity
prefactors approach more weakly (pinned fractions $0.21$ and $0.24$).

The Fisher information makes the same point structurally.  Eigendecomposing the
active Fisher matrix at each optimum yields spectra that span, on average,
$\sim$37 orders of magnitude (11.5--69.3 across conditions), with only 5--10 of
the 16 active directions lying within six decades of the stiffest one
(Fig.~\ref{fig:sloppy}).  The model is thus profoundly sloppy: the data
constrain essentially one stiff combination---dominated by the coupled
current-scale/gap coordinates ($g_0$ and the gap limits), which also carry the
lowest bootstrap CV---while series resistance and the velocity prefactors
populate the sloppiest directions.  This is the eigenbasis explanation of why
no single microscopic parameter, and only a few effective combinations, can be
read off a DC butterfly sweep.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{results/robustness/sloppy_spectrum.pdf}
\caption{Sloppiness of the Stanford parameterization.  Left: normalized Fisher
eigenvalue spectra for all twelve conditions, falling over tens of decades from
the stiff to the sloppy directions.  Right: mean participation of each active
parameter in the stiffest (best-determined) eigen-direction; it is a coupled
current-scale/gap combination, not any individual microscopic parameter.}
\label{fig:sloppy}
\end{figure}
...
single optimum but not repeatable under data resampling.  Profile-likelihood
scans completed for S1--S4 independently return ``flat/non-identifiable'' for
56 of 64 coordinates (88\%; 2 weak, 6 identifiable, the latter concentrated in
S4), corroborating the bootstrap reading.
...
bootstrap.  Rather than read trends off Fig.~\ref{fig:trends} by eye, we fit a
coded second-order response surface to each parameter and test it with a
center-point lack-of-fit $F$-test (Table~\ref{tab:doe-rsm}).  Only
$V_{0,\mathrm{SET}}$ shows a statistically significant deposition dependence:
it rises with oxygen ($\beta_{\mathrm{O}_2}=+0.21$, $p=0.027$) and falls with
power ($\beta_{P}=-0.18$, $p=0.044$), with $R^2=0.75$ and no significant
lack-of-fit ($p=0.12$).  $V_{0,\mathrm{RESET}}$ shows the same sign of oxygen
effect but does not reach significance ($p=0.052$; $R^2=0.61$), so we do
\emph{not} claim a RESET-side deposition trend.  Series resistance carries no
significant term at all ($R^2=0.24$, all $p>0.4$) and, as shown above, an
interval set entirely by the prior box; it must not be read as a deposition
trend even though a single fit appears to prefer a value.  This formal test is
the practical payoff of separating local identifiability from repeatability: it
tells the response-surface user which columns of the extracted parameter table
are admissible, and that only one of them currently is.

\begin{table}[!t]
\centering
\caption{Coded response-surface fit of the identifiable parameters versus
oxygen and power ($\pm1$ coded levels).  $p$-values are for the linear
coefficients; lack-of-fit (LoF) uses the four center-point replicates as pure
error.  Only $V_{0,\mathrm{SET}}$ is significant.}
\label{tab:doe-rsm}
\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
Param. & $\beta_{\mathrm{O}_2}$ & $p_{\mathrm{O}_2}$ & $\beta_{P}$ & $p_{P}$ &
$R^2$ & $p_{\mathrm{LoF}}$\\
\midrule
$V_{0,\mathrm{SET}}$  & $+0.213$ & $0.027$ & $-0.185$ & $0.044$ & 0.75 & 0.12\\
$V_{0,\mathrm{RESET}}$ & $+0.224$ & $0.052$ & $-0.166$ & $0.124$ & 0.61 & 0.08\\
$\log_{10}R_s$ & $-0.179$ & $0.443$ & $+0.151$ & $0.514$ & 0.24 & 0.33\\
\bottomrule
\end{tabular}
\end{table}

The replicated center points (S9--S12) also let us separate cycle-to-cycle from
device-to-device variability (Fig.~\ref{fig:varcomp}).  For \emph{every}
parameter the cycle-to-cycle coefficient of variation exceeds the
device-to-device coefficient, and the intraclass device fraction is below 0.2
(e.g.\ 0.04 for $V_{0,\mathrm{SET}}$, 0.06 for $R_s$).  Two conclusions follow:
the cycle bootstrap captures the dominant uncertainty (so device-to-device
intervals would add little), and the scatter among nominally identical
center-point devices is largely cycling, not fabrication---consistent with the
modest residual deposition signal recovered above.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{results/robustness/variance_components.pdf}
\caption{Cycle-to-cycle versus device-to-device coefficient of variation for
representative parameters, from the four replicated center-point devices
(S9--S12).  Cycle-to-cycle spread dominates for every parameter, so the
center-point scatter is mostly cycling rather than fabrication.}
\label{fig:varcomp}
\end{figure}
...
HRS observable that does not depend on the compact-model fit at all.
Figure~\ref{fig:schottky} plots the dynamic relative permittivity of the
post-RESET Schottky window, now computed per cycle and summarized as the cycle
median with a cycle-bootstrap 95\% interval (454 Schottky cycles in total).
The condition medians span 5.3 (S8) to 24.7 (S1), all inside the physically
admissible range, confirming that the dominant HRS conduction is
interface-limited Schottky emission across the entire DOE, not a fitting
artifact of a single device.  Because each value now carries a cycle-resolved
interval, $\varepsilon_r^{\mathrm{dyn}}$ functions as a model-free interface
fingerprint that remains informative exactly where the compact-model parameters
are non-identifiable---an automated, quantitative version of the mechanism
analysis previously performed by hand \cite{chowdhury2025mwscas}.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{results/paper_alt/hrs_schottky_trend.pdf}
\caption{Dynamic relative permittivity of the post-RESET HRS Schottky window,
computed per cycle and shown as the per-condition median with a cycle-bootstrap
95\% interval, versus oxygen (left) and power (right).  Shaded band:
admissibility interval $[1,60]$.  All condition medians (5.3--24.7) are
...
combined out-of-sample, bootstrap, and Fisher-eigenspectrum evidence, with
profile-likelihood scans completed for S1--S4 (88\% flat/non-identifiable) as
corroboration; a future release should complete profile likelihood at one
common optimum for all twelve conditions.  The cycle bootstrap is the primary
uncertainty, and the variance decomposition over the replicated center points
(S9--S12) shows cycle-to-cycle variation dominates device-to-device variation
for every parameter; nonetheless one selected device per non-center condition
limits the device- and wafer-level statistics, which a multi-device extension
would extend.  Finally, the reset-side $\gamma_0$ is structurally overridden
