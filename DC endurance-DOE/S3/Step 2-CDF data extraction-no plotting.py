...
def plot_cdf(data, xlabel=None, ylabel='CDF', title=None, output_file=None):
    ...
    plt.show()
df2 = pd.read_csv('S3-extracted_data.csv')
# data= df2['a']
#data = np.random.normal(loc=0, scale=1, size=1000)  # Example: 1000 random data points from a normal distribution
output_file = 'S3-Lrs.csv'

plot_cdf(df2['LRS'], xlabel='Data Points', ylabel='CDF', title='Cumulative Density Function', output_file=output_file)
