...
#     Robustness/novelty (B5): the post-RESET Schottky permittivity is now a
#     CYCLE-RESOLVED, model-free process observable.  We classify the Schottky
#     window of EVERY measured cycle (02c writes per-cycle slope + eps_r), then
#     report the cycle median with a cycle-bootstrap 95% CI instead of a single
#     medoid-window value.  This makes eps_r a process fingerprint with real
#     error bars even where the compact-model parameters are non-identifiable.
# ----------------------------------------------------------------------------
def _percentile_ci_median(x, n_boot=2000, seed=0):
    """Median and percentile-bootstrap 95% CI of the median of x."""
    ...
    return (float(np.median(x)), float(np.percentile(boots, 2.5)),
            float(np.percentile(boots, 97.5)))


def hrs_schottky_trend():
    print("[B] HRS Schottky dynamic permittivity vs O2% (cycle-bootstrap CIs)")
    rows = []
    for c in CONDS:
        o2, pw = DOE[c]
        pc_path = find_one(f"step02_{c}/*_regime_per_cycle.csv")
        eps_cyc = np.array([])
        if pc_path is not None:
            pc = pd.read_csv(pc_path)
            if "eps_r_dyn" in pc.columns:
                sel = pc[(pc.state == "RESET_HRS_POST") &
                         (pc.mechanism == "schottky")]
                eps_cyc = pd.to_numeric(sel["eps_r_dyn"],
                                        errors="coerce").dropna().to_numpy()
        med, lo, hi = _percentile_ci_median(eps_cyc)
        rows.append(dict(condition=c, o2_pct=o2, power_W=pw,
                         eps_r_dyn=med, eps_ci_lo=lo, eps_ci_hi=hi,
                         n_cycles=int(eps_cyc.size)))
    df = pd.DataFrame(rows).dropna(subset=["eps_r_dyn"])
    df.to_csv(os.path.join(OUT, "hrs_schottky_trend.csv"), index=False)
    print("  wrote results/paper_alt/hrs_schottky_trend.csv")
    print(f"  cycle-median eps_r range: {df.eps_r_dyn.min():.1f}-"
          f"{df.eps_r_dyn.max():.1f} over {df.n_cycles.sum()} schottky cycles")
    ...
    for ax, xkey, xlab, other in [
        ...
        for lev, mk, col in zip(sorted(df[other].unique()),
                                ...
                                ["#1f77b4", "#d62728", "#2ca02c", "#9467bd"]):
            d = df[df[other] == lev]
            yerr = np.vstack([np.clip(d.eps_r_dyn - d.eps_ci_lo, 0, None),
                              np.clip(d.eps_ci_hi - d.eps_r_dyn, 0, None)])
            ax.errorbar(d[xkey], d.eps_r_dyn, yerr=yerr, fmt=mk, color=col,
                        ms=4.5, capsize=2, lw=0.9, elinewidth=0.8,
    ...
    fig.suptitle("Interface (Schottky) dynamic permittivity of the HRS "
                 "vs deposition condition (cycle median $\\pm$ 95% CI)", fontsize=9)
