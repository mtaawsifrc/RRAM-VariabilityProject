...
def main() -> int:
    ...
    parser.add_argument(
        "--ensemble", default="",
        help="Optional per-cycle ensemble CSV; enables Stage-5 LOCO/bootstrap CIs.",
    )
    ...
    ensemble = (
        f"'{matlab_quote(Path(args.ensemble).expanduser().resolve())}'"
    ...
    script = (
        ...
        f"'{matlab_quote(config)}','{matlab_quote(outdir)}',{ensemble});"
    )
